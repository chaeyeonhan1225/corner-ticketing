...
class TicketTransferService:
    ...
    def request_transfer(self, giver: User, receiver: User, origin_ticket: UserTicket) -> bool:
        ...
        if (cache.set(f'ticket_transfer_{origin_ticket.uuid}', transfer_code, nx=True, timeout=5 * 60) and
                cache.set(cache_key, {'transfer_code': transfer_code,
    ...
    def receive_ticket(self, code: str, receiver: User):
        ...
        with transaction.atomic():
            ...
            new_ticket = UserTicket(
                owner=receiver,
                ticket=origin_ticket.ticket,
                giver=origin_ticket.owner
            )
            origin_ticket.save()
            new_ticket.save()
